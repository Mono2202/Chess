...
string ChessBoard::moveCheck(const BoardPosition& srcPos, const BoardPosition& destPos, const bool isWhite)
{
	...
	/*
	Removed this if condition, since the user can only press on the squares on the board,
	meaning there will never be an option to go out of bounds.
	...
	Removed the else since this is the first if condition now*/ if (srcPos == destPos)
